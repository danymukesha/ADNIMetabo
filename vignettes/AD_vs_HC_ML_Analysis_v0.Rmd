...
train_or_load <- function(model_name, train_func, reload = TRUE) {
...
ROC AUC scores of 0.848 and 0.845, respectively (Page 7). LASSO and
Random Forest also performed strongly with an AUC of 0.84, while Naive
Bayes was the least performant (AUC = 0.737). This hierarchy of
performance is consistent with the literature, where complex, non-linear
ensemble methods like XGBoost often excel in high-dimensional biological
datasets [1], and regularized linear models like LASSO and PLS are
particularly well-suited for collinear data, such as metabolomics
profiles [2]. The strong performance of PLS, a method designed for
dimensionality reduction in datasets with multicollinearity, underscores
the high degree of correlation expected among serum metabolites [3]. The
comparatively lower performance of Naive Bayes highlights the model's
inherent assumption of feature independence, which is frequently
violated in biological systems, leading to suboptimal performance when
dependencies exist among metabolites [4].
...
25 most influential metabolites for distinguishing AD from CN (Page
9-10). While the specific metabolites are not listed in the provided
excerpt, such analyses typically reveal disruptions in key biological
pathways in Alzheimer's disease. Previous metabolomic studies have
consistently implicated dysregulation in pathways such as phospholipid
metabolism (e.g., phosphatidylcholines, sphingomyelins), energy
metabolism (e.g., acylcarnitines, ketone bodies), amino acid metabolism
(e.g., branched-chain amino acids), and bile acid metabolism in AD
pathogenesis [5, 6, 7]. The ability of a data-driven approach to select
these metabolites reinforces their biological relevance and suggests
that the model is capturing meaningful metabolic signatures rather than
noise. Identifying these top features not only validates the model but
also provides crucial hypotheses for future biological validation
studies, potentially pointing to novel therapeutic targets or deeper
understanding of the metabolic underpinnings of AD [8].
...
instance, the LASSO model achieved a test AUC of 0.90 (Page 11),
compared to 0.89 for the model with all 196 features (Page 23). This
demonstrates that a significant portion of the predictive signal is
concentrated within a smaller subset of metabolites. This refinement is
critical in biomarker discovery, as a smaller, robust panel is far more
feasible and cost-effective to translate into a clinical assay [9]. It
also helps mitigate the risk of overfitting, a common challenge in
high-dimensional data analysis [10], by reducing the number of
...
# ggsave("results/roc_curves_testset_with_apoe.png", gg_roc_test, height = 5.7, width = 6.4, dpi = 700)1

gg_roc_test <- ggplot(roc_data_clinic #|> dplyr::filter(!model %in% c("LASSO with MMSE","LASSO only MMSE"))
                      ,aes(x = 1 - roc_df.fpr, y = roc_df.tpr, color = label)) +
    geom_line(size = 1) +
    geom_abline(linetype = "dashed", color = "gray40") +
    labs(
        x = "1 - Specificity", y = "Sensitivity",
        title = "ROC Curves on Test Set",
        subtitle = "Comparison of Machine Learning Models on Held-Out Data (Optimal Model m-151)",
        caption = "Binary classification: AD vs CN"
    ) +
    theme_bw() +
    theme(
    ...
    coord_fixed()
print(gg_roc_test)
ggsave("results/roc_curves_testset_with_apoe.png", gg_roc_test, height = 5, width = 6, dpi = 700)
```
