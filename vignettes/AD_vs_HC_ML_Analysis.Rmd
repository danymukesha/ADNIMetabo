...
ggplot(top_metabolite_summary, 
       aes(x = reorder(Class, n), y = n)) +
    geom_col(fill = "#1f78b4", width = 0.6) +
    geom_text(aes(label = n), 
              hjust = -0.1, size = 4.5, color = "black", family = "sans") +
    coord_flip(clip = "off") +
    scale_y_continuous(expand = expansion(mult = c(0, 0.15))) +
    labs(
        title = "Biochemical Classes of Top-Ranked Metabolites Identified by LASSO",
        subtitle = "Derived from the Biocrates MxP® Quant 500 panel",
        x = NULL,
        y = "Number of Selected Metabolites",
        caption = "LASSO-selected metabolites grouped by biochemical class (n = 630 total panel metabolites)."
    ) +
...
p1 <- ggplot(top_metabolite_summary, aes(x = reorder(Class, n), y = n)) +
  ...
  scale_y_continuous(expand = expansion(mult = c(0, 0.15))) +
  labs(
    title = "A. Biochemical Class Distribution",
...
p2 <- ggplot(bubble_data, aes(x = LASSO, y = PLS, size = Size, label = Metabolite)) +
  ...
  labs(
    title = "B. Variable Importance in LASSO vs PLS",
...
cv_performance_cleaned <- cv_performance[, lapply(.SD, function(x) if (is.numeric(x)) round(x, 2) else x)] |>
        dplyr::mutate(
        Model = case_when(
            Model == "lasso_reduced" ~ sub("lasso_reduced", "~150 metabolites", Model),
            Model == "lasso_reduced_withAPOE" ~ sub("lasso_reduced_withAPOE", "~150 metabolites + APOE", Model),
            Model == "lasso_onlyAPOE" ~ sub("lasso_onlyAPOE", "APOE alone", Model),
            Model == "lasso_reduced_withMMSE" ~ sub("lasso_reduced_withMMSE", "~150 metabolites + APOE + MMSE", Model),
