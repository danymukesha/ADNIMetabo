---
title: "ADNI metabolomics statistical analysis report"
...
vignette: >
  %\VignetteIndexEntry{ADNI_metabolomics_statistical_analysis}
...
```{r descriptive_plot, fig.width=7, fig.height=7, fig.align='center', out.width='80%'}
...
The analyzed cohort included 357 cognitively normal (CN) and 237 AD
participants from the ADNI database. The average age was similar between
groups (CN: 74.6 ± 5.8 years; AD: 74.9 ± 7.7 years), with no
statistically significant difference. Females were more prevalent in
both groups (CN: 68.7%, AD: 61.6%). As expected, APOE4 allele frequency
was higher in the AD group, consistent with known genetic risk profiles
[@corder1993].
...
All metabolomic data underwent preprocessing in line with established
standards. Missing values were imputed using the k-nearest neighbors
(k=10) method, followed by batch correction via the ComBat empirical
Bayes algorithm. Normalization was conducted using probabilistic
quotient normalization, and pareto scaling was applied to enhance
multivariate analysis sensitivity. The quality of the dataset was
robust, with a median coefficient of variation (CV) of 12.3% for quality
control samples and a post-imputation missing value rate of 2.1%,
indicating high technical reproducibility.

# PCA results

Principal component analysis (PCA) revealed that the first two
components explained 26.64% and 12.00% of the variance, respectively.
Partial separation between AD and CN groups was observed along PC1,
suggesting global metabolic shifts associated with disease.
Stratification by APOE4 status or sex did not produce marked separation,
but initial PCA identified batch effects, which were addressed during
preprocessing. Post-adjustment PCA plots showed improved separation
between diagnostic groups.

```{r pca1, fig.align='center', out.width='80%'}
...
For statistical modeling, we employed multivariate ANOVA with
Benjamini-Hochberg FDR correction, accounting for plate effects, age,
and sex a critical step also for the covariate interactions in
multi-omics studies.
...
```{r anovagraphs, messAge=FALSE, fig.align='center', out.width='80%'}
...
We performed a data adjustment process across multiple variables in a
dataset, by calculating predictions based on these models and subtracts
certain predicted effects (likely the main effects of the predictors)
from the original data. The process involves linear regression modeling
and selective removal of predictor influences from multiple variables.
The process removes the influence of these predictor variables from each
of the analyzed columns, which could be interpreted as a form of batch
correction or covariate adjustment. This technique is often used to
account for known sources of variation and isolate the effects of
interest.

The input dataset, referred to as data2, contains multiple columns
including: Plate, Sex, and Age with various measurement variables (from
column 8 onwards)
...
-   It aims to remove the effects of Plate, Sex, and Age from each
...
}), .SDcols = patterns("pvalue_adjusted")]
...
```{r afeter_effect_removal_by_ml, fig.align='center', out.width='80%'}
...
I chose the FC threshold of a 25% change is often large enough to
reflect real biological shifts, especially in metabolic pathways. This
is a moderate and commonly accepted threshold in metabolomics,
especially when we want to balance sensitivity and specificity. It
captures more hits while still avoiding tiny, likely insignificant
changes; which is good for pathway enrichment.

Volcano plot analysis identified 25 metabolites significantly associated
with AD, with 3 upregulated and 22 downregulated in affected
individuals. The most prominent changes were observed in lipid species,
including sphingolipids, glycerophospholipids, and carnitine esters.
These findings are consistent with previous studies implicating lipid
dysregulation—particularly involving plasmalogens and sphingomyelins—as
...
Receiver operating characteristic (ROC) analysis showed that individual
metabolites such as acetylornithine (AUC = 0.587) and 3-indoleacetic
acid (3-IAA, AUC = 0.565) had moderate predictive value. While single
metabolite performance was limited, prior evidence suggests that
multi-marker panels can substantially enhance diagnostic performance.
These findings align with known metabolic involvement in arginine and
tryptophan pathways and reinforce the role of mitochondrial dysfunction
as reflected in altered TCA cycle intermediates.
...
```{r, fig.width=5, fig.height=5, fig.align='center', out.width='80%'}
...
Pathway analysis identified four significantly impacted metabolic
routes: sphingolipid metabolism, glycerophospholipid metabolism,
arginine and proline metabolism, and tryptophan metabolism. Disruption
of lipid-related pathways emerged as the central feature, reinforcing
evidence from prior lipidomic investigations. Additionally,
perturbations in neurotransmitter precursor pathways and mitochondrial
bioenergetics suggest broader neurochemical and metabolic disruptions
contributing to AD pathophysiology.

# Conclusion

The resulting analysis identified 25 significantly altered metabolites
in AD patients, predominantly reflecting disruptions in lipid
metabolism. Biomarkers such as acetylornithine and 3-IAA emerged as
diagnostically relevant candidates. Pathway-level disturbances further
implicated neurotransmitter synthesis and mitochondrial dysfunction in
AD pathology. These results contribute to the growing consensus that
systemic metabolic dysfunction is integral to Alzheimer’s disease.
Future work should focus on validating these findings in multiple
independent cohorts, exploring their utility in preclinical stages, and
examining therapeutic modulation of implicated metabolic pathways.
